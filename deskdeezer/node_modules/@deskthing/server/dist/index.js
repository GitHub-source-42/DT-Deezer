...
__export(index_exports, {
  ...
  createDeskThing: () => createDeskThing
});
...
// ../deskthing-types/dist/apps/appSettings.js
var SETTING_TYPES;
...
// ../deskthing-types/dist/apps/appTasks.js
var STEP_TYPES;
...
// ../deskthing-types/dist/deskthing/mappings.js
var EventFlavor;
...
// src/imageUtils/imageHandler.ts
...
var ImageHandler = class {
   ...
   */
  async saveImageReference(url, appId, headers) {
   ...
   */
  async getImageSource(url, headers) {
   ...
   */
  isLocalPath(url) {
    return url.startsWith("file://") || url.startsWith("/") || url.match(/^[a-zA-Z]:\\/) !== null || url.startsWith("./") || url.startsWith("../");
  }
  /**
   ...
   */
  async getImageFromRemoteUrl(url, headers) {
    const maxRetries = 3;
    let attempt = 0;
    let lastError = null;
...
var DeskThingClass = class _DeskThingClass {
  constructor() {
     ...
     * @param callback
     */
    this.fetch = async (requestData, listenData, callback) => {
      if (!requestData.type) {
    ...
    this.postProcessMessage = async (data) => {
      if (import_worker_threads2.parentPort?.postMessage) {
  ...
  initializeListeners() {
    import_worker_threads2.parentPort?.on("message", async (data) => {
      switch (data.type) {
        case "data":
  ...
  async getData() {
    if (!this.appData?.data) {
      const data = await this.fetch(
        ...
        { type: DESKTHING_EVENTS.DATA, request: "" }
      );
      if (!data) {
  ...
  async deleteSettings(settingIds) {
    ...
    this.settings && this.notifyListeners(DESKTHING_EVENTS.SETTINGS, {
      type: DESKTHING_EVENTS.SETTINGS,
      payload: this.settings
    });
  ...
  async deleteData(dataIds) {
    ...
    this.appData?.data && this.notifyListeners(DESKTHING_EVENTS.DATA, {
  ...
  saveAppData(data, sync = true) {
    ...
    this.appData && this.notifyListeners(DESKTHING_EVENTS.APPDATA, {
      type: DESKTHING_EVENTS.APPDATA,
  ...
  saveSettings(settings, sync = true) {
    if (settings) {
      ...
      this.appData?.settings && this.notifyListeners(DESKTHING_EVENTS.SETTINGS, {
        type: DESKTHING_EVENTS.SETTINGS,
   ...
   */
  async encodeImageFromUrl(url, type = "jpeg", headers, retries = 0) {
    if (!url || typeof url !== "string") {
      throw new Error("Invalid URL provided");
    }
    try {
   ...
   */
  async saveImageReferenceFromURL(url, headers) {
    if (!this.manifest) {
      this.loadManifest();
    }
