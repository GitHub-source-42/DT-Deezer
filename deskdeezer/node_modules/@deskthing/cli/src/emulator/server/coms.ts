...
import { AppSettings, Client, LOGGING_LEVELS, APP_REQUESTS, DESKTHING_EVENTS } from "@deskthing/types"
...
type TypeHandler = {
  [key in APP_REQUESTS]: RequestHandler;
};
type RequestHandler = {
...
export const handleDataFromApp = async (
  ...
  if (Object.values(APP_REQUESTS).includes(appData.type as APP_REQUESTS)) {
    try {
      const handler = handleData[appData.type as APP_REQUESTS] || handleData.default;
...
const handleConnectionsRequest: HandlerFunction = async (app) => {
